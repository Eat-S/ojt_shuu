...
export async function addNewTradeInfo(tradeData: Omit<TradeInfo, 'id' | 'newName'> & { itemId?: string }): Promise<string> {
/**
 ...
 */
  ...
  const dataToStore: any = {
    ...
    isPurchase: tradeData.isPurchase,
    isCash: tradeData.isCash,
  };

  if (tradeData.date) {
    dataToStore.date = tradeData.date;
  }
  // This is where you would put the item's ID into the tradeinfo document's data.
  // Your description said: "tradeinfo(...): id:equals item's autogenerated id"
  // So, we assume `tradeData.itemId` holds this value.
  if (tradeData.itemId) {
    dataToStore.id = tradeData.itemId; // Storing item's ID in a field named 'id' within the tradeinfo doc
  }


  try {
    const docRef = await addDoc(tradesCollectionRef, dataToStore);
